...
2. The pairing URL (rendered as a QR code or opened directly) carries the daemon's public key in its URL fragment (`https://app.rambla.sh/#offer=...`). Fragments are not sent to the web server, so `app.rambla.sh` never sees the key.

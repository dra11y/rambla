...
To configure more than one preferred model or control the exact order, set `agents.metadataGeneration.providers` in `~/.rambla/config.json`. Your entries are tried before the built-in defaults.

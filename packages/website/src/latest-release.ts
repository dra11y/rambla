...
const LINUX_APPIMAGE_ASSET_PATTERN =
  /^Rambla-(?:\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)-)?x86_64\.AppImage$/;
...
function pickWindowsAssets(assets: GitHubAsset[]) {
  const x64Suffixed = assets.find((asset) => /Rambla-Setup-.*-x64\.exe$/.test(asset.name));
  const arm64 = assets.find((asset) => /Rambla-Setup-.*-arm64\.exe$/.test(asset.name));
  const legacy = assets.find(
    (asset) =>
      /Rambla-Setup-.*\.exe$/.test(asset.name) &&
...
function isReleaseInfo(value: unknown): value is ReleaseInfo {
  ...
  return (
    ...
    (record.linuxAppImageAsset === "Rambla-x86_64.AppImage" ||
      new RegExp(`^Rambla-${record.version.replaceAll(".", "\\.")}-x86_64\\.AppImage$`).test(
    ...
    (record.windowsX64Asset === null ||
      new RegExp(`^Rambla-Setup-${record.version.replaceAll(".", "\\.")}(?:-x64)?\\.exe$`).test(
        record.windowsX64Asset,
      )) &&
    (record.windowsArm64Asset === null ||
      new RegExp(`^Rambla-Setup-${record.version.replaceAll(".", "\\.")}-arm64\\.exe$`).test(

...
description: Rambla adds managed worktrees, native clients across desktop and mobile, and extensible daemon and client surfaces to a remote coding-agent workflow.
nav: Happy Coder
order: 53
---

# Rambla vs Happy Coder
...
Rambla supports the same two providers alongside OpenCode, Pi, ACP agents, and custom providers. It also manages workspaces, worktrees, services, pull-request review, and application plugins through the same daemon.

## Architecture

Rambla runs the agent inside its own daemon. The daemon owns the agent lifecycle, the worktree, and the dev servers. Clients connect over a websocket and drive the daemon.
...
Rambla runs each agent in its own git worktree. Each worktree gets its own dev server URL like `web.fix-auth.my-app.localhost`, so parallel agents don't fight for the same port.
...
Both products support voice interaction. Rambla can run speech-to-text and text-to-speech locally on the device.
...
See also: [Rambla vs Conductor](/alternatives/conductor), [Rambla vs Superset](/alternatives/superset), [Rambla vs OpenChamber](/alternatives/openchamber).

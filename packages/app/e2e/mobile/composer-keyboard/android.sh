...
APP_ID="${PASEO_COMPOSER_KEYBOARD_APP_ID:-sh.rambla.debug}"
...
adb shell am start \
  ...
  -d "exp+rambla://expo-development-client/?url=http%3A%2F%2F10.0.2.2%3A${METRO_PORT}" \
  "${APP_ID}" >/dev/null
sleep 3
...
adb shell am start \
  -a android.intent.action.VIEW \
  -d "rambla://h/${SERVER_ID}/agent/${agent_id}" \
  "${APP_ID}" >/dev/null
sleep 5

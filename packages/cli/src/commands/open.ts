...
function findDesktopApp(): string | null {
  if (process.platform === "darwin") {
    const candidates = [
      "/Applications/Rambla.app",
      path.join(homedir(), "Applications", "Rambla.app"),
    ];
  ...
  if (process.platform === "linux") {
    const candidates = [
      "/usr/bin/Rambla",
      "/opt/Rambla/Rambla",
      path.join(homedir(), "Applications", "Rambla.AppImage"),
    ];
  ...
  if (process.platform === "win32") {
    ...
    const candidate = path.join(localAppData, "Programs", "Rambla", "Rambla.exe");
